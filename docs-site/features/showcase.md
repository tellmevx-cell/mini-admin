...
![分析页](./screenshots/01-dashboard.png)
...
![审批中心](./screenshots/02-workflow-center.png)

### 消息通知中心

消息中心覆盖站内信、通知模板、通知策略、个人订阅和投递记录。

![消息通知中心](./screenshots/03-message-center.png)
...
![租户管理](./screenshots/04-tenant.png)
...
![用户管理](./screenshots/05-user.png)

### 角色管理

角色管理用于配置菜单权限、按钮权限和数据权限边界。

![角色管理](./screenshots/06-role.png)

### 菜单管理

菜单管理维护 Vben 动态路由、菜单层级、组件路径和权限码。

![菜单管理](./screenshots/07-menu.png)

### 权限诊断

权限诊断用于排查用户、角色、菜单、数据范围和缓存授权链路。

![权限诊断](./screenshots/08-permission-diagnostics.png)
...
![代码生成器](./screenshots/09-code-generator.png)

### 示例订单

示例订单展示普通业务模块如何接入工作流审批。

![示例订单](./screenshots/10-sample-order.png)

### 客户资料

客户资料是标准 CRUD 示例，可作为二开业务模块参考。

![客户资料](./screenshots/11-customer.png)
...
![文件管理](./screenshots/12-file-storage.png)
...
![系统监控](./screenshots/13-monitor.png)

### 告警中心

告警中心展示告警列表、告警规则和通知联动。

![告警中心](./screenshots/14-alert-center.png)

### 操作日志

操作日志记录接口审计、请求记录、实体变更和导出能力。

![操作日志](./screenshots/15-audit-log.png)
...
![登录日志](./screenshots/16-login-log.png)

### 安全中心

安全中心集中维护登录安全策略、密码策略、安全事件和在线会话。

![安全中心](./screenshots/17-security-center.png)
...
![项目运行管理](./screenshots/18-project-runtime.png)

### 定时任务

定时任务提供任务配置、手动执行、任务日志和运行状态。

![定时任务](./screenshots/19-scheduled-job.png)
