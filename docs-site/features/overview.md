...
flowchart LR
    ...
    B --> C["PageRegistry"]
    B --> D["数据权限"]
    C --> E["Dynamic API 业务页面"]
    F["SaaS 租户"] --> E
    G["工作流"] --> H["消息中心"]
    H --> I["站内信/邮件/短信/Webhook"]
    H --> R["SignalR 通知与聊天"]
    E --> J["审计日志"]
    E --> K["Local/S3/OSS/COS/MinIO"]
    L["代码生成器"] --> E
    M["事件总线"] --> E
    N["工作单元"] --> E
    O["接口限流"] --> A
...
| 平台内核 | Dynamic API、PageRegistry、RBAC + ABAC、版本化缓存 | 用统一元数据减少接口、菜单和权限样板代码 |
| 组织与字典 | 部门、岗位、字典、参数 | 支撑人员组织和业务枚举 |
| SaaS 租户 | 租户、套餐、初始化、资源配额、到期提醒、自动过期与续期 | 支撑多租户后台和租户级授权 |
| 工作流审批 | 定义、发布、发起、审批、转办、撤回、催办、抄送 | 支撑合同、采购、报销等审批场景 |
| 消息中心 | Scriban 模板、站内信、邮件、短信、Webhook、SignalR、在线聊天 | 支撑系统通知、业务通知和实时协作 |
| 开放平台 | OAuth2/OIDC、第三方应用、授权同意、AppKey/HMAC | 支撑外部应用和个人自动化安全接入 |
| 审计与安全 | 审计日志、实体变更、登录日志、安全中心 | 支撑合规、排障和追踪 |
| 监控与告警 | 系统监控、告警规则、计划任务 | 支撑运行态观察 |
| 文件存储 | 文件上传、下载、Local、S3、OSS、COS、MinIO | 支撑附件和多云对象存储 |
| 代码生成器 | 分层 CRUD、Dynamic API、PageRegistry、产物治理 | 降低新增模块成本 |
| 架构扩展 | 本地事件总线、工作单元、分层应用服务 | 支撑领域事件、事务边界和二开扩展 |
| 安全防护 | 登录安全、接口限流、安全事件、在线会话 | 降低撞库、误操作和异常访问风险 |
| 网关入口 | YARP 代理、灰度发布、TraceId、限流、熔断 | 支撑统一入口、渐进发布和后续微服务演进 |
...
6. 对外集成优先使用 OAuth2/OIDC 或个人 OpenAPI 凭证，不共享后台账号。
